...
## Scenarios
### Scenario 1: Translation Engine Port Contract
- **Given** the `TranslationEnginePort` interface in `com.lucalzt.mctranslator.domain.port.out`
- **When** inspected for dependencies
- **Then** it has no Spring or persistence framework imports and defines `translate(...)`.

### Scenario 2: Glossary Port Contract
- **Given** the `GlossaryPort` interface in `com.lucalzt.mctranslator.domain.port.out`
- **When** inspected for dependencies
- **Then** it has no Spring or persistence framework imports and defines `getTerms(...)`.

### Scenario 3: Translation Cache Port Contract
- **Given** the `TranslationCachePort` interface in `com.lucalzt.mctranslator.domain.port.out`
- **When** inspected for dependencies
- **Then** it has no Spring or persistence framework imports and defines `find(...)` and `save(...)`.

## Risks & Mitigations
- **Risk**: Coupling ports to infrastructure DTOs.
  - **Mitigation**: Ensure parameters and return types are domain models/records or pure domain primitives.
