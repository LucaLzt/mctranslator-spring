...
Modern alternative to TestRestTemplate. TestRestTemplate itself is not `@Deprecated` — it was relocated to `org.springframework.boot.resttestclient` in Boot 4 (add the `spring-boot-resttestclient` test dependency), but RestTestClient is the preferred API going forward:
