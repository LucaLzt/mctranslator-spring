...
description: 'Use ONLY when establishing general Spring Boot 4 project structure, Maven/Gradle dependencies, and configuration property scanning.'
---
...
- **Starters:** Use Spring Boot starters (e.g., `spring-boot-starter-webmvc`, `spring-boot-starter-data-jpa`) to simplify dependency management. Since Boot 4 the web starter is `spring-boot-starter-webmvc` (renamed from `spring-boot-starter-web`, which is deprecated).
...
- **Validation:** Use Jakarta Bean Validation 3.1 with annotations (`@Valid`, `@NotNull`, `@Size`) on DTOs to validate request payloads.
...
- **Unit Tests:** Write unit tests for services and components using JUnit 6 and a mocking framework like Mockito.
- **Integration Tests:** Use `@SpringBootTest` for integration tests that load the Spring application context.
- **Test Slices:** Use test slice annotations like `@WebMvcTest` (for controllers) or `@DataJpaTest` (for repositories) to test specific parts of the application in isolation. Since Boot 4, slices require the modular test starters: `spring-boot-starter-webmvc-test` for `@WebMvcTest` and `spring-boot-starter-data-jpa-test` for `@DataJpaTest`.
